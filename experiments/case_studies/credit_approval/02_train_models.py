...
class _IntersampleAttentionLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Reshape so that each token position becomes an independent "batch"
        # and the sample index becomes the sequence dimension.
        B, T, D = x.shape
        xt = self.norm(x).permute(1, 0, 2).contiguous()  # (T, B, D)
        # MHA with batch_first=True expects (batch, seq, D)
        # Here batch=T, seq=B — each token attends independently across samples
...
def main(
    ...
    out_path = Path(
        str(root_dir)
        + f"/experiments/case_studies/credit_approval/predictions/{dataset}/repeat_{repeat:04d}/fold_{fold:02d}"
    )
    ...
    if only is not None:
        ...
        print(f"[{dataset}] repeat={repeat} fold={fold} — fitting {only} only...")
